...
if [ -z "$CURRENT_PIPELINE_NUMBER" ] || [ "$CURRENT_PIPELINE_NUMBER" = "null" ]; then
  echo "WARNING: Could not determine current pipeline number from API. Allure will generate a fresh report."
  mkdir -p "$OUTPUT_DIR"
  exit 0
fi
...
if [ -z "$PREVIOUS_PIPELINE_ID" ] || [ "$PREVIOUS_PIPELINE_ID" = "null" ]; then
  ...
  if [ -z "$PREVIOUS_PIPELINE_ID" ] || [ "$PREVIOUS_PIPELINE_ID" = "null" ]; then
    echo "WARNING: No previous pipeline found. Allure will generate a fresh report."
    mkdir -p "$OUTPUT_DIR"
    exit 0
  fi
...
if [ -z "$WORKFLOW_ID" ] || [ "$WORKFLOW_ID" = "null" ]; then
  echo "WARNING: No workflow found for previous pipeline. Allure will generate a fresh report."
  mkdir -p "$OUTPUT_DIR"
  exit 0
fi
...
if [ -z "$JOB_NUMBERS" ]; then
  echo "WARNING: No jobs found in previous workflow. Allure will generate a fresh report."
  mkdir -p "$OUTPUT_DIR"
  exit 0
fi
...
if [ $ARTIFACTS_FOUND -eq 0 ]; then
  echo "WARNING: No artifacts found matching path '${ARTIFACT_PATH}'. Allure will generate a fresh report."
